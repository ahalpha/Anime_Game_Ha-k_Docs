...
- 可选择: 
  ...
  - `Think01AS` 拍照思考动作结束。<font size="2" color="#b6b6ba">&nbsp;(队友可见)&nbsp;</font>
...
- 设定的快捷键不为 `None` 时 仅在按住 [<font>**`重置快按键`**</font>](#重置快捷键) 时重置动画。

- 设定的快捷键为 `None` 时 则会一直重置动画。

- 若要一直重置动画 则设定此处为 `None` 并在 [热键](/cheat/bkebi-gc/08_hotkeys) 处设定按键 。
...
设定[<font>**`启用`**</font>](#启用-1)时[<font>**`浏览器`**</font>](#浏览器)所打开的链接。
...
设定的数值直接加到[<font>**`最大视距`**</font>](#最大视距)和[<font>**`最小视距`**</font>](#最小视距)上。
...
- 若要一直播放[<font>**`口型`**</font>](#口型)与[<font>**`情绪`**</font>](#情绪) 则设定此处为 `None` 并在 [热键](/cheat/bkebi-gc/08_hotkeys) 处设定按键 。
...
- 若要一直重置表情 则设定此处为 `None` 并在 [热键](/cheat/bkebi-gc/08_hotkeys) 处设定按键 。
...
启用后将不会显示造成的任何伤害 直到关闭后与下一次攻击造成伤害后一起显示。
